...
func (app *Config) MyPurchase(w http.ResponseWriter, r *http.Request) {
	...
	bookings, err := app.Repo.GetMyPurchases(timeoutCtx, requestPayload)
	if err != nil {
		app.errorJSON(w, err, nil)
		return
	}
...
func (app *Config) GetPurchaseRequest(w http.ResponseWriter, r *http.Request) {
	...
	bookings, err := app.Repo.GetPurchaseRequest(timeoutCtx, requestPayload)
	if err != nil {
		app.errorJSON(w, err, nil)
		return
	}
...
func (app *Config) MySubscriptionHistory(w http.ResponseWriter, r *http.Request) {
	...
	//extract the request body
	var requestPayload data.MySubscriptionHistoryPayload
	err := app.readJSON(w, r, &requestPayload)
	if err != nil {
		log.Printf("%v", err)
		app.errorJSON(w, err, nil)
	...
	bookings, err := app.Repo.GetMySubscriptionHistory(timeoutCtx, requestPayload)
	if err != nil {
		app.errorJSON(w, err, nil)
		return
	}
